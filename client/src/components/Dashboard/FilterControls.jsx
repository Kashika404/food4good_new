...
const FilterControls = ({ searchTerm, setSearchTerm, categoryFilter, setCategoryFilter, sortMethod, setSortMethod, categories }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-4 bg-white/60 rounded-xl shadow-sm">
      ...
      <div className="relative">
        ...
        >
          ...
          <option value="expiry-desc">Sort by Expiry (Latest)</option>
          <option value="title-asc">Sort by Name (A-Z)</option>
        </select>
        <FontAwesomeIcon icon={faSort} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/>
      </div>
    </div>
  );
};
