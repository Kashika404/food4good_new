...
const TaskCard = ({ task, onAccept }) => {
  // Styles for different task priorities or types
  const statusStyles = {
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-lg flex flex-col">
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex justify-between items-start mb-2">
          ...
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${statusStyles[task.urgency]}`}>
            {task.urgency}
          </span>
        </div>
        ...
        <div className="mt-2 text-sm text-neutral-600 space-y-2">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="font-semibold">From:</span> {task.from}
            </div>
          </div>
          <div className="flex items-start gap-3">
      ...
      <div className="px-5 pb-5 mt-auto">
          <button 
            ...
            className="w-full bg-orange-400 hover:bg-orange-600 text-white font-semibold py-2 rounded-full transition-colors shadow-md hover:shadow-lg">
              View Details & Accept
          </button>
      </div>
    </div>
